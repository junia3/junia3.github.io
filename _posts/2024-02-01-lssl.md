...
그리고 I/O 의 라플라스 변환 형태인 $i(s), y(s)$로 SISO system의 transfer function $G(s)$를 정의할 수 있게 된다. 하지만 해당 **transfer function**이 내포하는 어려움은 infinite dimensional하며, continous delay $\theta$를 모두 커버치기 불가능하다는 문제가 생긴다.
