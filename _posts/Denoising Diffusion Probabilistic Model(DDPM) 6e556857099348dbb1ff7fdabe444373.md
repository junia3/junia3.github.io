...
![score.png](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/score.png)
...
[https://www.jmlr.org/papers/volume6/hyvarinen05a/hyvarinen05a.pdf](https://www.jmlr.org/papers/volume6/hyvarinen05a/hyvarinen05a.pdf)
...
![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled.png)
...
![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%201.png)
...
![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%202.png)
...
![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%203.png)
...
![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%204.png)
...
DDPM에서 각 $t$에 대해 생성된 노이즈 첨가 샘플들은 이후에도 설명하겠지만 역과정을 학습하기 위한 조건부로 쓰인다. 즉, 우리가 얻고자 하는 생성 모델은 flow based model과 같이 표현하자면 $q(X_{t-1} | X_t)$와 같지만, 실제로는 이를 바로 얻을 수 없기 때문에 $p_{\theta}(X_{t-1} | X_t)$를 학습하고자 하는 것이다. Forward와 반대로 Reverse는 노이즈로부터 노이즈를 제거하는 작업이 되는데, 결국 특정 신호에 대해서 그보다 매우 작은 가우시안 형태의 잡음을 지속적으로 더하는 과정이나 빼는 과정은 학습되는 분포에 있어서 크게 다르지 않다는 것을 알 수 있기 때문에, 실질적으로 노이즈로부터 샘플을 만들어내는 과정을 모방할 수 있게 되는 것이다. 논문에 나온 표현을 인용하면,
...
$\mathcal{L} \le \mathbb{E}_{x_T}\left[-\log \left(\frac{p_\theta(x_0 \vert x_T)}{q (x_T \vert x_0)} \right) \right]$
...
![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%205.png)
...
![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%206.png)

Tractable한 항에 대한 정리는 위와 같다. 위 공식에 대한 유도는 논문을 참고하면 단순히 gaussian 분포의 조건부로 해결 가능한 수식이다.

![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%207.png)

따라서 결국에 위와 같은 식을 최적화하는 것이 목표가 되고, 각 step $t$에서 다음 방향으로의 gaussian 분포를 예측하게 된다. 앞선 VAE 글에서도 소개했었지만 분포를 예측하는 과정에서는 미분이 불가능하므로 이를 reparameterizationg하면,

![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%208.png)
...
![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%209.png)

그게 바로 baseline과 다른 요 방식. 성능이 더 잘나왔다고 한다.

## Limitations

![Untitled](Denoising%20Diffusion%20Probabilistic%20Model(DDPM)%206e556857099348dbb1ff7fdabe444373/Untitled%2010.png)
